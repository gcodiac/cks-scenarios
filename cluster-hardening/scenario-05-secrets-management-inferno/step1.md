# Use secrets to store sensitive data — Inferno
...
Create or update only the requested Secret, avoid printing values, restrict RBAC, and verify that an application receives the value without exposing it in logs or command history.
...
**Security focus:** Treat the Secret as data with an access policy: inspect references, mount or environment behavior, RBAC, and the fact that base64 is not encryption.
